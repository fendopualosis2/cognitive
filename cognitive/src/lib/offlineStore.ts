  ...
  static async syncWithServer(): Promise<boolean> {
    try {
      ...
      if (queue.length > 0 || activePatId) {
        if (activePatId) {
          const routines = this.getRoutine(activePatId);
        ...
        if (session.role === 'PATIENT') {
           const p = this.getPatients().find(x => x.id === session.patientId || session.caretakerId);
           ...
           const c = this.getCaretakers().find(x => x.id === session.patientId || session.caretakerId);
      ...
      if (session.role === 'PATIENT') query += `&patientId=${session.patientId || session.caretakerId}`;
      if (session.role === 'CAREGIVER') query += `&caretakerId=${session.patientId || session.caretakerId}`;
      ...
      if (res.ok) {
         ...
         if (data.targetCaretaker) this.addCaretaker(data.targetCaretaker);
         if (data.assignedPatients) this.savePatients(data.assignedPatients);
         if (data.activePatId) {
            this.saveRoutine(data.routines || [], data.activePatId);
