...
app.get('/api/auth/check-username', (req: any, res: any) => {
  ...
  try {
    ...
    if (role === 'PATIENT') {
      ...
      const newPatient: PatientProfile = {
        ...
        pin: await bcrypt.hash(profile.pin || password.slice(0, 4), 10),
...
app.post('/api/auth/logout', (req: any, res: any) => { res.clearCookie('token'); res.json({ success: true }); });

// Link patient to caregiver (by Patient Key, Phone, or Username)
app.post('/api/caretakers/:id/link-patient', requireAuth, (req: any, res: any) => { if (req.user.id !== req.params.id) return res.status(403).json({ error: 'Forbidden' });
  try {
...
app.post('/api/patients/:id/link-caregiver', requireAuth, (req: any, res: any) => { if (req.user.id !== req.params.id && !hasPatientAccess(req.user, req.params.id)) return res.status(403).json({ error: 'Forbidden' });
  try {
...
app.post('/api/patients/:id/unlink-caregiver', requireAuth, (req: any, res: any) => { if (req.user.id !== req.params.id && !hasPatientAccess(req.user, req.params.id)) return res.status(403).json({ error: 'Forbidden' });
  try {
...
app.get('/api/routines/:patientId', requireAuth, (req: any, res: any) => { if (!hasPatientAccess(req.user, req.params.patientId)) return res.status(403).json({ error: 'Forbidden' });
  res.json(ServerDB.getRoutines(req.params.patientId));
});

app.post('/api/routines/:patientId', requireAuth, (req: any, res: any) => { if (!hasPatientAccess(req.user, req.params.patientId)) return res.status(403).json({ error: 'Forbidden' });
...
app.get('/api/reminders/:patientId', requireAuth, (req: any, res: any) => { if (!hasPatientAccess(req.user, req.params.patientId)) return res.status(403).json({ error: 'Forbidden' });
  res.json(ServerDB.getReminders(req.params.patientId));
});

app.post('/api/reminders/:patientId', requireAuth, (req: any, res: any) => { if (!hasPatientAccess(req.user, req.params.patientId)) return res.status(403).json({ error: 'Forbidden' });
...
app.get('/api/memories/:patientId', requireAuth, (req: any, res: any) => { if (!hasPatientAccess(req.user, req.params.patientId)) return res.status(403).json({ error: 'Forbidden' });
  res.json(ServerDB.getMemories(req.params.patientId));
});

app.post('/api/memories/:patientId', requireAuth, (req: any, res: any) => { if (!hasPatientAccess(req.user, req.params.patientId)) return res.status(403).json({ error: 'Forbidden' });
...
app.delete('/api/memories/:patientId/:memoryId', requireAuth, (req: any, res: any) => { if (!hasPatientAccess(req.user, req.params.patientId)) return res.status(403).json({ error: 'Forbidden' });
  const updated = ServerDB.deleteMemory(req.params.patientId, req.params.memoryId);
  res.json(updated);
});

// Synchronized Sessions & Real-time Reports APIs
app.get('/api/sessions/:patientId', requireAuth, (req: any, res: any) => { if (!hasPatientAccess(req.user, req.params.patientId)) return res.status(403).json({ error: 'Forbidden' });
  res.json(ServerDB.getSessions(req.params.patientId));
});

app.post('/api/sessions/:patientId', requireAuth, (req: any, res: any) => { if (!hasPatientAccess(req.user, req.params.patientId)) return res.status(403).json({ error: 'Forbidden' });
...
app.delete('/api/patients/:id', requireAuth, (req: any, res: any) => {
  try {
...
app.post('/api/patients', requireAuth, (req: any, res: any) => {
...
app.post('/api/caretakers', requireAuth, (req: any, res: any) => {
...
app.patch('/api/caretakers/:id/key', requireAuth, (req: any, res: any) => { if (req.user.id !== req.params.id) return res.status(403).json({ error: 'Forbidden' });
  try {
    const { caregiverKey } = req.body;
...
app.patch('/api/patients/:id/key', requireAuth, (req: any, res: any) => { if (req.user.id !== req.params.id && !hasPatientAccess(req.user, req.params.id)) return res.status(403).json({ error: 'Forbidden' });
  try {
...
// -------------------------------------------------------------
app.post('/api/ai/daily-report', requireAuth, async (req: any, res: any) => { if (req.user.role !== 'CAREGIVER') return res.status(403).json({ error: 'Forbidden' });
  try {
    const {
      patient,
      sessions = [],
...
// -------------------------------------------------------------
app.post('/api/ai/suggest-routine', requireAuth, async (req: any, res: any) => { if (req.user.role !== 'CAREGIVER') return res.status(403).json({ error: 'Forbidden' });
  try {
